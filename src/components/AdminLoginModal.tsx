...
import { ShieldCheck, Mail, Lock, X, ArrowRight, AlertCircle, Sparkles } from 'lucide-react';
import { loginAdminWithEmail, checkIsAdmin } from '../services/firestoreService';
...
export const AdminLoginModal: React.FC<AdminLoginModalProps> = ({
  ...
  const handleEmailSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!email || !password) {
      setErrorMsg('Por favor, informe o e-mail e a senha administrativa.');
      return;
    }
    ...
    try {
      ...
      if (!isAdmin && email.trim() !== 'guseletrico2005@gmail.com') {
        setErrorMsg('Esta conta não possui privilégios de administrador.');
        setLoading(false);
        return;
      }
      onLoginSuccess(user);
  ...
  const handleGoogleAdmin = async () => {
    ...
    try {
      ...
      const isAdmin = await checkIsAdmin();
      if (!isAdmin && user.email !== 'guseletrico2005@gmail.com') {
        setErrorMsg('Esta conta Google não possui privilégios de administrador.');
        setLoading(false);
        return;
      }
      onLoginSuccess(user);
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/60 backdrop-blur-xs animate-in fade-in duration-200">
      <div className="bg-white rounded-3xl border border-[#EFE2E7] shadow-2xl max-w-md w-full p-6 sm:p-8 relative space-y-5">
        ...
        <div className="relative flex items-center justify-center">
          ...
          <span className="bg-white px-3 text-[11px] text-[#8E6A77] absolute">ou</span>
        </div>
        ...
        >
          ...
          <span>Acesso Administrativo via Google</span>
        </button>
